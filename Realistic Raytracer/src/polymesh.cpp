...
void PolyMesh::do_construct(char *file, Transform *transform)
{
  ...
  for (i = 0; i < vertex_count; i += 1)
  {
    ...
    extents->introduceVertex(vertex[i]);
    
  }

  hitbox = new Box(*extents);
  hitbox_misses = 0;

  cout << "Polymesh Hitbox: " << *hitbox << endl;
...
Vertex PolyMesh::barycentric(Vertex point, Vertex a, Vertex b, Vertex c)
{
	Vertex result;
	float u;
	float v;
	...
	Vector v0 = Vector(b - a);
	Vector v1 = Vector(c - a);
	Vector v2 = Vector(point - a); 
...
void PolyMesh::triangle_intersection(Ray ray, Hit &hit, int which_triangle)
{
  ...
  //Now interpolate using barycentric..

  Vertex BC_coeff = this->barycentric(
    hit.position,
    Vertex(v0.x, v0.y, v0.z, 1.0f),
    Vertex(v1.x, v1.y, v1.z, 1.0f),
    Vertex(v2.x, v2.y, v2.z, 1.0f)
  );

  int a_num = triangle[which_triangle][0];
  int b_num = triangle[which_triangle][1];
  int c_num = triangle[which_triangle][2];

  hit.normal.x = (BC_coeff.x * vertex_normal[a_num].x) + (BC_coeff.y * vertex_normal[b_num].x) + (BC_coeff.z * vertex_normal[c_num].x);
  hit.normal.y = (BC_coeff.x * vertex_normal[a_num].y) + (BC_coeff.y * vertex_normal[b_num].y) + (BC_coeff.z * vertex_normal[c_num].y);
  hit.normal.z = (BC_coeff.x * vertex_normal[a_num].z) + (BC_coeff.y * vertex_normal[b_num].z) + (BC_coeff.z * vertex_normal[c_num].z);
  hit.normal.normalise();
...
{
  Hit hitbox_hit = Hit();
  hitbox->intersection(ray, hitbox_hit);
  if (hitbox_hit.flag == false) 
  {
    hitbox_misses++;
    return; //ray doesnt intersect hitbox, so dont do triangle checks to save computation
  }
